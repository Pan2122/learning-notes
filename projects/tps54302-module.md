...
输出电压由反馈电阻 **R1**(上拉) 和 **R2** (下拉) 决定，公式如下：
...
1.  **电感 (L1)**：必须选用 **饱和电流 > 4A** 的功率电感。
    * ❌ **错误示范**：0603/0805 叠层电感（电流太小，会瞬间饱和导致短路）。
    * ✅ **本项目选用**：1040 封装 (10mm x 10mm) 或 0630 封装的屏蔽功率电感，感值 10µH。
2.  **输入电容 (C1/C2)**：必须耐高压。
...
* **功率回路 (Power Loop)**：输入电容 **CIN** (C2/C3) **紧贴** 芯片的 VIN 和 GND 引脚。这是减小高频辐射的关键。
...
### 设计理念
* **风格**：Industrial / Cyberpunk。
* **元素**：高对比度的色块、粗线条边框、加上“IN/OUT”流向指示。
* **配色**：最终选择了 **紫色阻焊 (Purple Solder Mask)** 搭配 **白色丝印**，并在焊盘处做沉金处理。
...
如果测试通过，这将作为一个标准的 Power Block 复用到我未来的项目中。
